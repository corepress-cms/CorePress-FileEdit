...
function wp_print_theme_file_tree( $tree, $level = 2, $size = 1, $index = 1 ) {
	global $relative_file, $stylesheet, $corepress_theme_file_editor_path;
	...
	if ( is_array( $tree ) ) {
		...
		$url      = add_query_arg(
			...
			self_admin_url( $corepress_theme_file_editor_path )
		);
		?>
...
add_action( 'load-tools_page_corepress_file_editor_theme', 'corepress_file_editor_theme_add_help_tab' );
add_action( 'load-themes_page_corepress_file_editor_theme', 'corepress_file_editor_theme_add_help_tab' );

function corepress_file_editor_theme_add_help_tab() {
    ...
    $screen->set_help_sidebar(
        ...
        '<p>' . __( '<a href="https://developer.wordpress.org/advanced-administration/wordpress/edit-files/">Documentation on Editing Themes</a>' ) . '</p>' .
...
function corepress_file_editor_theme_display_editor() {
        ...
        if ( is_wp_error( $r ) ) {
            ...
            wp_redirect(
                add_query_arg(
                    ...
                    admin_url( $corepress_theme_file_editor_path )
                )
            );
            exit;
        }
    ...
    <div class="alignright">
        <form action="<?php echo $corepress_theme_file_editor_path; ?>" method="get">
            <input type="hidden" name="page" value="corepress_file_editor_theme" />
            ...
            <?php if ( $theme->parent() ) : ?>
                <li class="howto">
                    ...
                    printf(
                        ...
                        sprintf(
                            '<a href="%s">%s</a>',
                            self_admin_url( ( wp_is_block_theme() ? 'tools.php' : 'themes.php' ) . '?page=corepress_file_editor_theme&theme=' . urlencode( $theme->get_template() ) ),
    ...
    if ( $error ) :
        ...
        ?>
        <form name="template" id="template" action="<?php echo $corepress_theme_file_editor_path; ?>" method="post">
