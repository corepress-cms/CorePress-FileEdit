...
$corepress_plugin_file_editor_path = ( wp_is_block_theme() && ! is_multisite() ) ? 'tools.php' : 'plugins.php';
/**
...
function wp_print_plugin_file_tree( $tree, $label = '', $level = 2, $size = 1, $index = 1 ) {
	...
	if ( is_array( $tree ) ) {
        ...
        global $corepress_plugin_file_editor_path;
		$url = add_query_arg(
			array(
            ...
            self_admin_url( $corepress_plugin_file_editor_path )
		);
		?>
...
add_action( 'load-tools_page_corepress_file_editor_plugin', 'corepress_file_editor_plugin_add_help_tab' );
add_action( 'load-plugins_page_corepress_file_editor_plugin', 'corepress_file_editor_plugin_add_help_tab' );

function corepress_file_editor_plugin_add_help_tab() {
...
function corepress_file_editor_plugin_display_editor() {
    if ( ( is_multisite() && ! is_network_admin() ) || ! current_user_can( 'edit_plugins' ) ) {
    ...
    <div class="alignright">
        <form action="<?php echo $corepress_plugin_file_editor_path; ?>" method="get">
            <label for="plugin" id="theme-plugin-editor-selector"><?php _e( 'Select plugin to edit:' ); ?> </label>
            <input type="hidden" name="page" value="corepress_file_editor_plugin" />
    ...
    <form name="template" id="template" action="<?php echo $corepress_plugin_file_editor_path; ?>" method="post">
